...
// root into a temp dir; MAESTRO_DAEMON_HOME keeps the run files out of
// ~/.maestro-daemon.
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'maestro-daemon-js-'))
const bin = process.env.MAESTRO_DAEMON_TEST_BIN ?? path.join(tmp, 'maestro-daemon')
process.env.MAESTRO_DAEMON_HOME = path.join(tmp, 'home')
process.env.MAESTRO_DAEMON_BIN = bin
delete process.env.MAESTRO_DEVICE
delete process.env.MAESTRO_DAEMON
...
before(async () => {
  if (!process.env.MAESTRO_DAEMON_TEST_BIN) {
    execFileSync('go', ['build', '-o', bin, '.'], { cwd: repoRoot, stdio: 'inherit' })
  }
  daemon = await MaestroDaemon.connect({ name: 'jstest', idleTimeout: 120 })
...
describe('connect', () => {
  ...
  test('a second connect reuses the running daemon', async () => {
    const d2 = await MaestroDaemon.connect({ name: 'jstest' })
  ...
  test('spawn: false rejects with DAEMON_UNAVAILABLE when nothing runs', async () => {
    await rejects(
      () => MaestroDaemon.connect({ name: 'jstest-none', spawn: false }),
      (err) => {
  ...
  test('a bad name is a TypeError', async () => {
    await assert.rejects(() => MaestroDaemon.connect({ name: 'bad name!' }), TypeError)
  })

  test('a missing binary is DAEMON_UNAVAILABLE', async () => {
    await rejects(
      () => MaestroDaemon.connect({ name: 'jstest-nobin', bin: path.join(tmp, 'nope') }),
...
describe('devices', () => {
  ...
  test('a device attached by another daemon is DEVICE_IN_USE', async () => {
    const other = await MaestroDaemon.connect({ name: 'jstest2', idleTimeout: 120 })
    try {
      await rejects(
        () => other.attach(MOCK),
        (err) => {
...
describe('detach and shutdown', () => {
  ...
  test('shutdown() stops the process and removes the run files', async () => {
    ...
    await rejects(
      () => MaestroDaemon.connect({ name: 'jstest', spawn: false }),
      (err) => assert.ok(isMaestroError(err, 'DAEMON_UNAVAILABLE')),
    )
    daemon = undefined
  })
