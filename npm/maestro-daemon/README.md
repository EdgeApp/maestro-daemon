...
```js
import { MaestroDaemon } from 'maestro-daemon'

const m = await MaestroDaemon.attach({ device: '29271FDH200ABP', appId: 'co.edgesecure.app' })
...
**daemon** (`maestro-daemon serve`) that keeps the device driver open; every
later call — from this process, a later `node` run, or the `maestro-daemon`
...
- **Same daemon, three doors.** The JS library, the `maestro-daemon` CLI and
...
```sh
npm install maestro-daemon
```

The Go binary comes from a per-platform optional dependency
(`maestro-daemon-darwin-arm64`, `-darwin-x64`, `-linux-arm64`, `-linux-x64`).
If you install with `--no-optional`, or run on another platform, put
`maestro-daemon` on `PATH` or point `MAESTRO_DAEMON_BIN` at it.
...
`MAESTRO_RUNNER_HOME=$HOME/.maestro-runner` — or point `MAESTRO_DAEMON_BIN`
at `~/.maestro-runner/bin/maestro-daemon` after `make build`.
...
```ts
static connect(opts?: ConnectOptions): Promise<MaestroDaemon>
...
| option | meaning |
| --- | --- |
| `name` | daemon name (default `$MAESTRO_DAEMON`, else `default`) |
...
| `bin` | path to the binary (else `$MAESTRO_DAEMON_BIN`, the platform package, `PATH`) |
...
| variable | |
| --- | --- |
| `MAESTRO_DAEMON` | default daemon name |
| `MAESTRO_DEVICE` | default device id for `attach()` |
| `MAESTRO_DAEMON_BIN` | path to the Go binary |
| `MAESTRO_DAEMON_HOME` | run-file root (default `~/.maestro-daemon`); sockets, logs and reports live under `run/<name>/` |
...
```js
const d = await MaestroDaemon.connect({ name: 'pair-test' })
...
The package also installs the `maestro-daemon` command:

```sh
npx maestro-daemon launchApp co.edgesecure.app --device 29271FDH200ABP
npx maestro-daemon tapOn Login
npx maestro-daemon commands           # every one-shot command
npx maestro-daemon tapOn --help       # a command's fields
npx maestro-daemon status
npx maestro-daemon stop
```

See the [maestro-daemon repository](https://github.com/EdgeApp/maestro-daemon)
...
`MAESTRO_DAEMON_TEST_BIN` to use one you already built.
