...
// Edge CLI's ensureEngine): spawn `maestro-daemon serve --daemon <name>`
...
export interface SpawnOptions {
  ...
  extraArgs?: string[]
  /** Path of the maestro-daemon binary (else MAESTRO_DAEMON_BIN, bundled, PATH). */
  bin?: string
    ...
    if (!pidAlive(existing.pid)) {
      ...
      throw MaestroError.of(
        ...
        `daemon "${name}" (pid ${existing.pid}) exists but does not answer on ${existing.socketPath}; ` +
          `stop it with \`maestro-daemon stop --daemon ${name}\``,
      )
    }
  }
  if (opts.spawn === false) {
