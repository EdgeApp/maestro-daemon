// Locating the maestro-daemon Go binary.
//
// Order: $MAESTRO_DAEMON_BIN → the per-platform npm package installed as an
// optional dependency → `maestro-daemon` on $PATH. Nothing is downloaded at
...
export const PACKAGE_BY_TARGET: Record<string, string> = {
  'darwin-arm64': 'maestro-daemon-darwin-arm64',
  'darwin-x64': 'maestro-daemon-darwin-x64',
  'linux-arm64': 'maestro-daemon-linux-arm64',
  'linux-x64': 'maestro-daemon-linux-x64',
}
...
export function bundledBinary(): string | undefined {
  ...
  // A registry install puts it beside maestro-daemon in the consumer's
  ...
  // a symlink outside the package (…/bin/maestro-daemon → …/lib/node_modules/
  // maestro-daemon/bin/maestro-daemon.js), and resolving from the link's own
  ...
  for (const from of roots) {
    try {
      ...
      const bin = path.join(path.dirname(manifest), 'bin', 'maestro-daemon')
...
/** First `maestro-daemon` on $PATH, or undefined. */
export function pathBinary(): string | undefined {
  const dirs = (process.env.PATH ?? '').split(path.delimiter).filter(Boolean)
  for (const dir of dirs) {
    const p = path.join(dir, 'maestro-daemon')
...
export function binaryPath(explicit?: string): string {
  ...
  throw MaestroError.of(
    'DAEMON_UNAVAILABLE',
    pkg
      ? `maestro-daemon binary not found: install ${pkg} (it is an optional dependency of maestro-daemon; ` +
          `installs with --no-optional skip it), put maestro-daemon on PATH, or set ${ENV_BIN}`
      : `maestro-daemon has no prebuilt binary for ${target()}; build it from source ` +
          `(go build -o maestro-daemon .) and set ${ENV_BIN} or put it on PATH`,
  )
}
