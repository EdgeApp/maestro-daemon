// MaestroD: a handle on one daemon process. `connect()` finds the named
...
export interface ConnectOptions extends SpawnOptions {
  /** Daemon name (default `$MAESTRO_D`, else 'default'). One daemon per name. */
  name?: string
  /**
