...
a few lines marked with a `// maestro-daemon` comment or a self-contained
...
| File | Change | Lines |
| --- | --- | --- |
| `pkg/cli/cli.go` | `app.Commands = append(app.Commands, daemonCommands()...)`; app name, usage text and version printer say `maestro-daemon` | ~15 |
...
| `Makefile` | `BINARY_NAME=maestro-daemon`; `gen` / `gen-check` targets; `build` replaces the installed binary's inode (a running daemon plus an in-place `cp` gets the next exec SIGKILLed on macOS) | ~14 |
| `README.md` | Fork banner at the top | ~17 |
| `.gitignore` | Ignore the `maestro-daemon` binary and `npm/dist-daemon/` | 6 |
...
| `pkg/daemon/gen/` | Generator: reflects over the step structs → `commands_gen.go`, `npm/maestro-daemon/src/generated/commands.ts`, `docs/daemon/commands.md` |
...
| `npm/maestro-daemon/` | The JavaScript library |
| `npm/build-daemon-npm.sh` | Cross-compiles the binary into per-platform npm packages and packs the library (`npm/build-npm.sh` is upstream's, for maestro-runner's own package) |
| `.github/workflows/daemon.yml` | The fork's checks and `daemon-v*` release workflow (`ci.yml` is upstream's) |
...
`maestro-runner` version in `npm/maestro-daemon/package.json`'s description.
