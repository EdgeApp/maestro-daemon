# The maestro-daemon daemon

maestro-daemon is [maestro-runner](https://github.com/devicelab-dev/maestro-runner)
...
| --- | --- | --- |
| CLI | `maestro-daemon tapOn Login --device 29271FDH200ABP` | [cli.md](cli.md) |
...
4. **The daemon goes away on its own.** After 30 minutes without a request
   ...
   simulator or emulator *it* booted, and exits. `maestro-daemon stop` does
...
Everything lives under `~/.maestro-daemon/run/<name>/` (`$MAESTRO_DAEMON_HOME`
overrides the root):
...
directory. `make build` installs it to `~/.maestro-runner/bin/maestro-daemon`
...
```json
{
  ...
  "error": {
    ...
    "details": {
      "reportDir": "/Users/me/.maestro-daemon/run/default/reports/29271FDH200ABP/20260910-101500",
...
```sh
git clone https://github.com/EdgeApp/maestro-daemon
cd maestro-daemon
make build          # → ~/.maestro-runner/bin/maestro-daemon (and copies drivers/)
export PATH="$HOME/.maestro-runner/bin:$PATH"
maestro-daemon doctor
```

Or from npm, which brings the binary along: `npm install maestro-daemon`
(see [js.md](js.md)).

## A first session

```sh
maestro-daemon devices                                   # what this machine can see
maestro-daemon attach --device 29271FDH200ABP --app-id co.edgesecure.app
maestro-daemon launchApp                                 # uses the attach --app-id
maestro-daemon tapOn "Get started"
maestro-daemon get screenshot -o now.png
maestro-daemon assertVisible "Create account" || echo "not there (exit $?)"
maestro-daemon stop
```
