...
//	maestro-daemon tapOn --id btn --index 2   → - tapOn: {id: btn, index: 2}
//	maestro-daemon tapOn Login --optional     → - tapOn: {text: Login, optional: true}
//	maestro-daemon swipe --start.x 50% …      → - swipe: {start: {x: 50%}, …}
//	maestro-daemon repeat --yaml loop.yaml    → - repeat: <file contents>
//	maestro-daemon back                       → - back
...
// Every other command needs a value or a field, so a bare `maestro-daemon
...
func commandHelp(spec daemon.CommandSpec) string {
	...
	case spec.ValueLess:
		...
		return b.String()
	case spec.Scalar != "":
		fmt.Fprintf(&b, "A bare value sets %q: maestro-daemon %s VALUE  ≡  - %s: VALUE\n", spec.Scalar, spec.Name, spec.Name)
	}
	if valueOptional[spec.Name] {
		fmt.Fprintf(&b, "The value may be omitted: maestro-daemon %s  ≡  - %s\n", spec.Name, spec.Name)
	}
	if spec.Compound {
...
func buildStepValue(spec daemon.CommandSpec, p *parsedArgs) (any, error) {
	...
	for _, f := range fields {
		if top, _, _ := strings.Cut(f.key, "."); len(types) > 0 && types[top] == "" {
			return nil, usage("%s has no field %q (see `maestro-daemon %s --help`)", spec.Name, top, spec.Name)
		}
		var v any = true
		if !f.bare {
			var err error
