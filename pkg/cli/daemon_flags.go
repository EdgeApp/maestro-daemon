...
// daemon_flags.go holds the flag table shared by every maestro-daemon
...
// `maestro-daemon tapOn Login --device X --team-id T` accepts exactly the
// flags `maestro-daemon attach` does.
...
type daemonOpts struct {
	Name   string // daemon name (--daemon → MAESTRO_DAEMON → default)
...
var (
	...
	daemonSpawnFlags = []dflag{
		...
		intFlag("idle-timeout", "Seconds of inactivity before the daemon exits (0 = never; default 1800)", []string{"MAESTRO_DAEMON_IDLE_TIMEOUT"},
		...
		strFlag("http", "Also listen on this TCP address (host:port) for the REST API", []string{"MAESTRO_DAEMON_HTTP"},
			func(o *daemonOpts, v string) { o.HTTPAddr = v }),
		strFlag("token", "Bearer token required on the TCP listener", []string{"MAESTRO_DAEMON_TOKEN"},
	...
	attachFlags = []dflag{
		...
		attachStr("output-dir", "Report/artifact directory (default ~/.maestro-daemon/run/<name>/reports/<device>/<ts>)", []string{"MAESTRO_OUTPUT"},
